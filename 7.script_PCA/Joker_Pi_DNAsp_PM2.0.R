...
REF = "~/Documents/Solution/Transcriptomics/Coverage/CaZl1_PerGene/CaZL1.cds.fa"
species = "Pepper"
Colspecies = "#FF8C00"
ChrLOC=read.table("/Users/stephaniearnoux/Documents/Solution/references/CaZL1.Gene_Loc.tab")
setwd(PATH_STAT)

## Give sample details 
pop2 = "C.annuum" 
pop3 = "C.annuum var. glab."

Species= "Pepper"
title = "PM_DNAsp"

CropCol_Light = "#FF8C00"
CropCol_Dark = "#EC3F00"

## Settle the file names
StatCropNS = "PM_DNAsp_CROP_NS_dadi.noMiss.VCF.Clean.out" 
StatCropSyn = "PM_DNAsp_CROP_SYN_dadi.noMiss.VCF.Clean.out" 

StatWildNS = "PM_DNAsp_WILD_NS_dadi.noMiss.VCF.Clean.out"
StatWildSyn = "PM_DNAsp_WILD_SYN_dadi.noMiss.VCF.Clean.out"
...
################# Remove the LD   ################# 
################################################### 
chr <- substr(dtChrLOC$V1,7,8)
LDdtCrop <- cbind(dtChrLOC,chr=as.numeric(as.character(chr)))
GenesLD <- LDdtCrop[((LDdtCrop$chr==1)&(LDdtCrop$V2>=60000000)&(LDdtCrop$V2<=160000000)| 
                            (LDdtCrop$chr==2)&(LDdtCrop$V2>=0)&(LDdtCrop$V2<=80000000)| 
                            (LDdtCrop$chr==3)&(LDdtCrop$V2>=100000000)&(LDdtCrop$V2<=220000000)| 
                            (LDdtCrop$chr==4)&(LDdtCrop$V2>=60000000)&(LDdtCrop$V2<=180000000)| 
                            (LDdtCrop$chr==5)&(LDdtCrop$V2>=60000000)&(LDdtCrop$V2<=160000000)| 
                            (LDdtCrop$chr==6)&(LDdtCrop$V2>=60000000)&(LDdtCrop$V2<=180000000)| 
                            (LDdtCrop$chr==7)&(LDdtCrop$V2>=40000000)&(LDdtCrop$V2<=150000000)| 
                            (LDdtCrop$chr==8)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=120000000)| 
                            (LDdtCrop$chr==9)&(LDdtCrop$V2>=30000000)&(LDdtCrop$V2<=180000000)| 
                            (LDdtCrop$chr==10)&(LDdtCrop$V2>=30000000)&(LDdtCrop$V2<=120000000)| 
                            (LDdtCrop$chr==11)&(LDdtCrop$V2>=40000000)&(LDdtCrop$V2<=140000000)| 
                            (LDdtCrop$chr==12)&(LDdtCrop$V2>=60000000)&(LDdtCrop$V2<=170000000)),]$V1 
...
#### DO NOT FILTER THE PREVIOUS BADGENES if you want to get fisher tests on DEG
#FdtWild[FdtWild$chr==9&FdtWild$DEG!=0] #436
#FdtWild[FdtWild$chr!=0&FdtWild$DEG!=0] #7073
...
names(itagLength) <- substr(names(itagLength),1,15) # not needed if you use seqinR
# head(d$ITAG)
...
names(down) <- gene.names #downselected genes

length(up[up==1])  #A 206 44
length(down[down==1]) #B 331 88
...
hist(WildPiNS_and_PiSyn$PiNpiS,nclass=100, col='dimgray',main='',xlab=expression(paste(pi,"N/",pi,"S",sep="")))
hist(CropPiNS_and_PiSyn$PiNpiS,nclass=100,add=TRUE, col=Colspecies,main='',xlab='')
...
tiff(file=imageoutput,height = 10, width = 13, units = 'cm',res = 300)
hist(WildPiNS_and_PiSyn$PiNpiS,ylim = c(0,500), xlim=c(0,20),nclass=5000, col='dimgray',main='',xlab=expression(paste(pi,"N/",pi,"S",sep="")))
hist(CropPiNS_and_PiSyn$PiNpiS,ylim = c(0,500), xlim=c(0,20),nclass=5000,add=TRUE, col=Colspecies,main='',xlab='')
...
ks.test(d$PiNPiS_Crop,d$PiNPiS_Wild, alternative = "greater") # D^+ = 0.19794, p-value < 2.2e-16
