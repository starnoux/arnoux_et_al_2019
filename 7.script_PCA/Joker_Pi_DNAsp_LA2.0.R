...
species = "Tomato"
Colspecies = "firebrick2"
ChrLOC=read.table("/Users/stephaniearnoux/Documents/Solution/references/ITAG3.2_gene_Loc.tab")
setwd(PATH_STAT)
## Give sample details 
pop2 = "S. lycopersicum" 
pop3 = "S. pimpinellifolium"

Species= "Tomato"
title = "LA_ALL_DNAsp"

CropCol_Light = "firebrick2"
CropCol_Dark = "darkred"

## Settle the file names
StatCropNS = "LA_DNAsp_CROP_NS_dadi.noMiss.VCF.Clean.out" 
StatCropSyn = "LA_DNAsp_CROP_SYN_dadi.noMiss.VCF.Clean.out" 

StatWildNS = "LA_DNAsp_WILD_NS_dadi.noMiss.VCF.Clean.out"
StatWildSyn = "LA_DNAsp_WILD_SYN_dadi.noMiss.VCF.Clean.out"
...
################# Remove the LD   ################# 
################################################### 
chr <- substr(dtChrLOC$V1,6,7)
LDdtCrop <- cbind(dtChrLOC,chr=as.numeric(as.character(chr)))

GenesLD <- LDdtCrop[((LDdtCrop$chr==1)&(LDdtCrop$V2>=15000000)&(LDdtCrop$V2<=65000000)| 
                       (LDdtCrop$chr==2)&(LDdtCrop$V2>=0)&(LDdtCrop$V2<=20000000)| 
                       (LDdtCrop$chr==3)&(LDdtCrop$V2>=150000000)&(LDdtCrop$V2<=40000000)| 
                       (LDdtCrop$chr==4)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=45000000)| 
                       (LDdtCrop$chr==5)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=55000000)| 
                       (LDdtCrop$chr==6)&(LDdtCrop$V2>=5000000)&(LDdtCrop$V2<=25000000)| 
                       (LDdtCrop$chr==7)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=50000000)| 
                       (LDdtCrop$chr==8)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=45000000)| 
                       (LDdtCrop$chr==9)&(LDdtCrop$V2>=12000000)&(LDdtCrop$V2<=55000000)| 
                       (LDdtCrop$chr==10)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=50000000)| 
                       (LDdtCrop$chr==11)&(LDdtCrop$V2>=12000000)&(LDdtCrop$V2<=45000000)| 
                       (LDdtCrop$chr==12)&(LDdtCrop$V2>=10000000)&(LDdtCrop$V2<=40000000)),]$V1 
GenesLD <- droplevels(GenesLD)
OUTGenesLD<-paste(title,"_GenesWithinLD.txt",sep="")
write.table(levels(GenesLD), OUTGenesLD, row.names = FALSE, col.names = "GENE")
## Get the locations 
pdtCrop <- data.table(CropPiNS_and_PiSyn, key = "Gene")
dtCrop <- pdtCrop[dtChrLOC]
chr <- substr(dtCrop$Gene,6,7)
...
maxPiCrop = quantile(d$PiNPiS_Wild, 0.9, na.rm=TRUE)[[1]]
...
PiC.thresh=quantile(d$PiNPiS_Crop, 0.95,na.rm=TRUE)[[1]]
#PiC.threshmin=0.05
PiW.thresh=quantile(d$PiNPiS_Wild, 0.95,na.rm=TRUE)[[1]]
#PiW.threshmin=0.05
ilength=itagLength
...
names(up) <- gene.names #upSelected genes A
length(up[up==1])
...
length(up[up==1])  #A 68 9
length(down[down==1]) #B 623 98
...
########
Up.table <- data.frame(cbind(names(up),as.character(up),as.numeric(as.character(substr(names(up),6,7)))))
colnames(Up.table) <- c("gene","A","chr")
Down.table <- data.frame(cbind(names(down),as.character(down),as.numeric(as.character(substr(names(down),6,7)))))
...
hist(WildPiNS_and_PiSyn$PiNpiS,nclass=200, col='dimgray',main='',xlab=expression(paste(pi,"N/",pi,"S",sep="")))
hist(CropPiNS_and_PiSyn$PiNpiS,nclass=30,add=TRUE, col="firebrick2",main='',xlab='')
...
tiff(file=imageoutput,height = 10, width = 13, units = 'cm',res = 300)
hist(WildPiNS_and_PiSyn$PiNpiS,ylim = c(0,500), xlim=c(0,20),nclass=800, col='dimgray',main='',xlab=expression(paste(pi,"N/",pi,"S",sep="")))
hist(CropPiNS_and_PiSyn$PiNpiS,ylim = c(0,500), xlim=c(0,20),nclass=100,add=TRUE, col=Colspecies,main='',xlab='')
title(main= paste(Species),lwd=1.8,lty=5,cex.main=1.4)
...
plot(delta.pi.dist$PiNPiS_Crop~delta.pi.dist$PiNPiS_Wild, cex.axis=1,pch=20,cex.lab=1.4,xlab =expression(paste(pi,"N/",pi,"S Wild",sep="")),ylab = expression(paste(pi,"N/",pi,"S Crop",sep="")), col = ifelse((delta.pi.dist$PiNPiS_Crop > PiC.thresh | delta.pi.dist$PiNPiS_Wild > PiW.thresh  ),'dimgray',Colspecies),las=1)
#xaxt="n", yaxt="n"
...
tiff(file=imageoutput,height = 10, width = 10, units = 'cm',res = 300)
par(mar=c(4.2,5.1,3.1,2.1))
plot(delta.pi.dist$PiNPiS_Crop~delta.pi.dist$PiNPiS_Wild, cex.axis=1,pch=20,cex.lab=1.4,xlab =expression(paste(pi,"N/",pi,"S Wild",sep="")),ylab = expression(paste(pi,"N/",pi,"S Crop",sep="")), col = ifelse((delta.pi.dist$PiNPiS_Crop > PiC.thresh | delta.pi.dist$PiNPiS_Wild > PiW.thresh  ),'dimgray',Colspecies),las=1, xlim=c(0,10),ylim=c(0,10))
#xaxt="n", yaxt="n"
#xlim=c(0,0.07), ylim=c(0,0.07), yaxt="n", xlim=c(0,0.1),xaxt="n",
#axis(1, at=c(0,0.02,0.04,0.06,0.08,0.1), cex.axis=0.8,las=1) #xaxis
#axis(2, at=c(0,0.02,0.04,0.06,0.08,0.1), cex.axis=0.8,las=1) #0.8
...
ks.test(d$PiNPiS_Crop,d$PiNPiS_Wild, alternative = "greater") #  0.17544, p-value < 2.2e-16
ks.test(d$PiNPiS_Crop,d$PiNPiS_Wild, alternative = "less") # D^- = 0, p-value = 1
