...
parser.add_argument("-gff",           type=str,       default="SMEL_V3.2016_11_01.gff.gene",                         help="input gff name, here only the mRNA should be kept (default: %(default)s)!")
parser.add_argument("-mapin",           type=str,       default="MM_CSFILTER_MD2018.recodePruned.map",                         help="output name (default: %(default)s)!")
parser.add_argument("-temp",           type=str,       default="MM_CSFILTER_MD2018.recodePruned.map.temp",                         help="temporary file name name (default: %(default)s)!")
parser.add_argument("-fin",           type=str,       default="MM_CSFILTER_MD2018.recodePruned.map.fin",                         help="output name (default: %(default)s)!")
parser.add_argument("-chrom",           type=str,       default="SMEL_0",                         help="output name (default: %(default)s)!")

args=parser.parse_args()

#with open (gff, 'r+b') as gff, open(mapin, 'r+b') as mapin,  open(temp, 'w') as temp:
...
temp = open(temp, 'r+b')
fin = open(fin, 'w')
#with open (mapin, 'r+b') as mapin, open(temp, 'r+b') as temp,  open(fin, 'w') as fin:
...
with open (mapin, 'r+b') as mapin, open(temp, 'r+b') as temp,  open(fin, 'w') as fin:
    ...
    for line in mapin:
        ...
        ID_line = line[1].split("_")[0]+'_'+line[1].split("_")[1]
        Loc_on_gene = line[1].split("_")[2]
